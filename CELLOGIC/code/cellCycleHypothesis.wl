...
(* ::Input::Initialization:: *)
cellCycleHypothesis::usage="This package is contains all modules used to test the cell cycle transciptional filter hypothesis Last Edited March 2021";


(* ::Input::Initialization:: *)
oneFullCycleV01::usage="
Used for the manuscript tittled: Control of tissue development by cell cycle dependent transcriptional filtering;
This Fucntion tests the effects of cell cycle duration, gene length.  
:=[tissue_,timestep_,cellCycleOpts_,genome_,transcriptionPara_] 
 improved half-life by using dataset associations new to Mathematica 12, improved speed by functionalising more steps and using association. Also by condensing the number of transcripts in the cell list -> {1,1,1,1,2,3,4,1} to a list that carries the total number-> {5,1,1,1} and association functions helped memory alocation.
   Now genome ploidy can be tested in the system: if the organism is ploidy level two it will carry two copies of each gene Last Edited March 2021";


(* ::Input::Initialization:: *)
oneFullCycleV12::usage="This Fucntion tests the effects of cell cycle duration, gene length and grN motifs. 
:=[tissue_,timestep_,cellCycleOpts_,genome_,transcriptionPara_] 
and added transcript competition and improved half-life by using dataset associations new to Mathematica, improved speed by functionalising more steps and using association. Also by condensing the number of transcripts in the cell list -> {1,1,1,1,2,3,4,1} to a list that carries the total number-> {5,1,1,1} and association functions helped memory alocation.
   Now genome ploidy can afftect the system: if the organism is ploidy level two it will carry two copies of each gene and can activate or repress these copies individually  Last Edited March 2020";
...
(* ::Input::Initialization:: *)
oneFullCycleV12[tissue_,timestep_,cellCycleOpts_,genome_,transcriptionPara_]:=Module[{ploidy,oneCycle,shortestToMphase,cellCycles,newtissue,cpos,icell,cyclePhase,tempCell,tCell,newcellcylceduration,tempgenome,lineage},
(*
...
newtissue=tissue;
newtissue[[All,4,5]]=shortestToMphase;
(*Print["before loop"];*)
(*Print[newtissue];*)(*for each cell currently in the tissue*)(*cellCR --\[Rule] center, radius, divisions,cellCycleInfo {,phase, Length of  last cycle,next cycle}, {{cycle length Now},{cycle Length Next}} (*These will be -100 if they are not holding any value, since zero may mean something*)
...
tempgenome=newtissue[[icell,6,2]];
...
(*Print["cell competition"];*)
If[(newtissue[[icell,4,4]]==0&&Union[newtissue[[icell,7,1]]]!={0}&&transcriptionPara[[1]]!={}) (*occurs only if the cell just divided*),
...
lineage={timestep,shortestToMphase, tempgenome,newtissue[[icell,6,2]],newtissue[[icell,7,1]],{},{},newtissue[[icell,4,3]],{},{}};
newtissue[[icell,9]]=lineage;
,
(*Mphase*)
	
(*move transcripts out of temp*)
If[newtissue[[icell,7,2]]!= {},
...
newtissue[[icell,3,3]]=cellRelatedFunctions`getunitDirectionOfDivision[icell,spaceParameters[[2]],newtissue,{},polarityOpt];*)

(*divide, increase divisions*)
(*creates 2 new cells, in a 2D space is not important*)
(*Print["cell division"];*)
...
(*Print["cell cycle change"];*)
(******add stem like??*****)
newcellcylceduration=cellRelatedFunctions`cellcycleChangeOPT[{newtissue[[icell,5,1,#]],cellCycleOpts[[1]],cellCycleOpts[[2]],cellCycleOpts[[4]]},cellCycleOpts[[3]]]&/@Range[1,cyclePhase];

If[cellCycleOpts[[3]]==-6||cellCycleOpts[[3]]==-7||cellCycleOpts[[3]]==-8,
(*this is the case where two daughtercells have a proportion of the parental cell*)
newcellcylceduration=Transpose[newcellcylceduration];
tempCell[[1,5,1]]=newcellcylceduration[[1]];tempCell[[2,5,1]]=newcellcylceduration[[2]];,
tempCell[[1,5,1]]=newcellcylceduration;tempCell[[2,5,1]]=newcellcylceduration;

];

(*Print[newcellcylceduration];*)
(*accumulated cell cycle time*)
tempCell[[1,4,3]]=Total[tempCell[[1,5,1]]/. _?Negative->0];tempCell[[2,4,3]]=Total[tempCell[[2,5,1]]/. _?Negative->0];
(*Print["rand dis of Transcripts"];*)
...
tempCell[[2,2]]=1;(*cell size stay at 1*)tempCell[[1,8]]=tempCell[[1,8]]<>"_"<>ToString[icell]<>"-1";(*name of the cell*)tempCell[[2,8]]=tempCell[[2,8]]<>"_"<>ToString[icell]<>"-2";
(*name of the cell*)
(*Print["linege 2"];*)
lineage=Join[lineage[[1;;4]],{lineage[[5]],tempCell[[1,7,1]],tempCell[[2,7,1]],lineage[[6]],tempCell[[1,4,3]],tempCell[[2,4,3]]}];

tempCell[[1,9]]=lineage;
tempCell[[2,9]]=lineage;
(*lineage = {timestep, shortesttoMphase, parental genotype initially,parental genotype after interaction, parental phenotype, offspring1 phenotype,offspring2 phenotype, parental cell cycle, offspring1 cell cycle duration,offspring2 cell cycle duration}*)
newtissue[[icell]]=tempCell[[1]];newtissue=AppendTo[newtissue,tempCell[[2]]];(*closepack*)
];(*if m phase (4) or not \[LessEqual]3*)
];(*for each cell*)
(*Print[transcriptionPara[[1]]];
Print[lineage];*)
